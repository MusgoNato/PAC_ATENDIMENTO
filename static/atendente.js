import {alertPersonalizado, confirmPersonalizado} from './utils.js';

/*------------------Atendente------------------*/

// Tornando o escopo das funções globais para serem utilizadas pelo onclick()
window.alertPersonalizado = alertPersonalizado;
window.chamarCliente = chamarCliente;
window.removerClienteDaFila = removerClienteDaFila;
    ...
    filaLista.innerHTML = fila.map((customer, index) => `
        <div class="queue-item ${index === 0 ? 'next-customer' : ''} ${customer.tipo === "PRIORITARIO" ? 'priority' : ''}">
            ...
            <div class="container-botoes-atendimento">
                ...
                <button class="funcao-btn-atendimento" style="background-color: rgba(255, 0, 0, 1); color: white;" onclick="removerClienteDaFila(${customer.id}, '${customer.numero}')">Remover</button>
            </div>
        </div>
    `).join('');
